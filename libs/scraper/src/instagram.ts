...
export class InstagramScraper {
  ...
  static async open(config: ScraperConfig): Promise<InstagramScraper> {
  ...
  async fetchRecentPosts(
    handle: string,
    opts: { since: Date | null; maxPosts?: number },
  ): Promise<ScrapedPost[]> {
  ...
  async waitBetweenHandles(): Promise<void> {
  ...
  private async performLogin(page: Page): Promise<void> {
    ...
    if (await this.hasChallenge(page)) {
      throw new Error(
        'Instagram presented a challenge/checkpoint. Run once with IG_HEADLESS=false ' +
