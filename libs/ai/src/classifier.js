...
import { classifierBatchResponseSchema, } from '@app/shared-types';
import { DEFAULT_MAX_TOKENS, DEFAULT_MODEL, MAX_POSTS_PER_BATCH, MIN_CONFIDENCE, } from './config.js';
 ...
 */
export async function classify(config, posts, options = {}) {
    if (posts.length === 0)
        return [];
...
async function classifyBatch(client, config, posts) {
...
function renderBatchUserMessage(posts) {
...
function parseJsonFromText(text) {
    ...
    try {
        ...
        throw new Error(`Classifier returned non-JSON response: ${err.message}\n---\n${body.slice(0, 500)}`);
    }
}
function* chunkPosts(items, size) {
